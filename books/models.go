...
func OneBook(r *http.Request) (Book, error) {
	...
	rows := config.DB.QueryRow("SELECT * FROM books WHERE isbn = $1", isbn)
...
func UpdateBook(r *http.Request) (Book, error) {
	...
	if bk.Isbn == "" || bk.Title == "" || bk.Author == "" || p == "" {
		return bk, errors.New("400. Bad Request. all the fields must be filled completely/")
	}

	f64, err := strconv.ParseFloat(p, 32)
	if err != nil {
		return bk, errors.New("406. Not acceptable. Enter for price")
	}
	bk.Price = float32(f64)
